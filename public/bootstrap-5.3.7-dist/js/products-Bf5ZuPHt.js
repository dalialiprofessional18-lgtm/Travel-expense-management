...
document.addEventListener("alpine:init", () => {
  o.data("productTable", () => ({
    ...
    isLoading: !1,
    chartsInitialized: !1,
    stats: { total: 0, inStock: 0, lowStock: 0, totalValue: 0 },
    categoryStats: [],
    init() {
      this.loadSampleData(),
        this.filterProducts(),
    ...
    calculateStats() {
      (this.stats.total = this.products.length),
        (this.stats.inStock = this.products.filter((t) => t.stock > 20).length),
        (this.stats.lowStock = this.products.filter(
          (t) => t.stock > 0 && t.stock <= 20
        ).length),
        (this.stats.totalValue = this.products.reduce(
          (t, s) => t + s.price * s.stock,
          0
        ));
      const e = {};
      this.products.forEach((t) => {
        ...
        (this.categoryStats = Object.entries(e).map(([t, s]) => ({
          name: t.charAt(0).toUpperCase() + t.slice(1),
          count: s,
          percentage: Math.round((s / this.products.length) * 100),
          color: this.getCategoryColor(t),
        })));
    },
    getCategoryColor(e) {
      return (
    ...
    filterProducts() {
      (this.filteredProducts = this.products.filter((e) => {
        const t =
          ...
          s = !this.categoryFilter || e.category === this.categoryFilter,
          i =
            !this.stockFilter ||
            (this.stockFilter === "in-stock" && e.stock > 20) ||
            (this.stockFilter === "low-stock" &&
              e.stock > 0 &&
              e.stock <= 20) ||
            (this.stockFilter === "out-of-stock" && e.stock === 0);
        return t && s && i;
      })),
        this.sortProducts(),
        (this.currentPage = 1);
    ...
    sortProducts() {
      this.filteredProducts.sort((e, t) => {
        let s = e[this.sortField],
          i = t[this.sortField];
        return (
          this.sortField === "price" || this.sortField === "stock"
            ? ((s = parseFloat(s)), (i = parseFloat(i)))
            : this.sortField === "created"
            ? ((s = new Date(s)), (i = new Date(i)))
    ...
    toggleAll(e) {
      e
        ? (this.selectedProducts = this.paginatedProducts.map((t) => t.id))
        : (this.selectedProducts = []);
    },
    bulkAction(e) {
      if (this.selectedProducts.length === 0) return;
      const t = this.products.filter((s) =>
        this.selectedProducts.includes(s.id)
      );
      switch (e) {
        case "publish":
          t.forEach((s) => {
        ...
        case "delete":
          confirm(
            ...
            ((this.products = this.products.filter(
              (s) => !this.selectedProducts.includes(s.id)
            )),
            this.filterProducts(),
            this.calculateStats(),
    ...
    duplicateProduct(e) {
      const t = {
        ...e,
        id: Math.max(...this.products.map((s) => s.id)) + 1,
        name: e.name + " (Copy)",
        sku: e.sku + "-COPY",
        status: "draft",
        created: new Date().toISOString().split("T")[0],
      };
      this.products.unshift(t),
        this.filterProducts(),
        this.calculateStats(),
    ...
    deleteProduct(e) {
      confirm(`Are you sure you want to delete "${e.name}"?`) &&
        ((this.products = this.products.filter((t) => t.id !== e.id)),
        this.filterProducts(),
        this.calculateStats(),
    ...
    exportProducts() {
      const e =
          `data:text/csv;charset=utf-8,Name,SKU,Category,Price,Stock,Status,Created
` +
          this.filteredProducts.map(
            (i) =>
              `"${i.name}","${i.sku}","${i.category}","${i.price}","${i.stock}","${i.status}","${i.created}"`
          ).join(`
`),
        t = encodeURI(e),
        s = document.createElement("a");
      s.setAttribute("href", t),
        s.setAttribute("download", "products.csv"),
    ...
    initSalesChart() {
      const e = document.getElementById("salesChart");
      if (!e) {
      ...
      try {
        const t = {
          series: [{ name: "Sales", data: [65, 78, 85, 92, 88, 95, 102] }],
        ...
        console.error("Error rendering sales chart:", t);
      }
    },
    initCategoryChart() {
      const e = document.getElementById("categoryChart");
      if (!e) {
      ...
      try {
        const t = {
          series: this.categoryStats.map((i) => i.count),
          chart: { type: "donut", height: 200 },
          labels: this.categoryStats.map((i) => i.name),
          colors: this.categoryStats.map((i) => i.color),
        ...
        console.error("Error rendering category chart:", t);
      }
    },
    get paginatedProducts() {
      const e = (this.currentPage - 1) * this.itemsPerPage,
        t = e + this.itemsPerPage;
      return this.filteredProducts.slice(e, t);
    },
    get totalPages() {
