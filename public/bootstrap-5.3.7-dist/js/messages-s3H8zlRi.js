import { m as i } from "./bootstrap.bundle.min.js";
  ...
  i.data("messagesComponent", () => ({
      ...
      if (!this.searchQuery.trim())
        ...
        this.filteredConversations = this.conversations.filter(
          (t) =>
            ...
            t.lastMessage.toLowerCase().includes(e) ||
    ...
    selectConversation(e) {
      (this.selectedConversation = e),
        (this.currentMessages = [...e.messages]),
        (e.unread = 0),
        window.innerWidth < 992 && (this.sidebarVisible = !1),
        this.$nextTick(() => {
          this.scrollToBottom();
        });
    },
    sendMessage() {
      ...
      const t = this.conversations.findIndex(
        (s) => s.id === this.selectedConversation.id
      );
      t !== -1 &&
        (this.conversations[t].messages.push(e),
        (this.conversations[t].lastMessage = this.newMessage.trim()),
        (this.conversations[t].lastMessageTime = "now")),
        (this.newMessage = ""),
        this.$nextTick(() => {
          this.scrollToBottom();
        }),
    ...
    simulateResponse() {
      this.selectedConversation &&
        ...
        setTimeout(() => {
          ...
          this.currentMessages.push(t),
            (this.selectedConversation.lastMessage = t.text),
            (this.selectedConversation.lastMessageTime = "now");
          const s = this.conversations.findIndex(
            (a) => a.id === this.selectedConversation.id
          );
          s !== -1 &&
            (this.conversations[s].messages.push(t),
            (this.conversations[s].lastMessage = t.text),
            (this.conversations[s].lastMessageTime = "now")),
            this.$nextTick(() => {
              this.scrollToBottom();
            });
    ...
    addEmoji(e) {
      (this.newMessage += e), (this.showEmojiPicker = !1);
    },
    toggleAttachment() {
    ...
    archiveConversation() {
      this.selectedConversation &&
        this.showNotification(
          `${this.selectedConversation.name} archived`,
          "success"
        );
    },
    deleteConversation() {
      this.selectedConversation &&
        confirm(
          `Delete conversation with ${this.selectedConversation.name}?`
        ) &&
        ((this.conversations = this.conversations.filter(
          (e) => e.id !== this.selectedConversation.id
        )),
        this.filterConversations(),
    ...
    simulateActivity() {
      setInterval(() => {
        ...
        Math.random() > 0.7 && (e.online = !e.online);
      }, 1e4),
        setInterval(() => {
          if (Math.random() > 0.8) {
            const e =
            ...
            (e.unread += 1),
              (e.lastMessage = t[Math.floor(Math.random() * t.length)]),
