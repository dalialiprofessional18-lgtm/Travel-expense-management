import { m as a } from "./bootstrap.bundle.min.js";
document.addEventListener("alpine:init", () => {
  a.data("helpComponent", () => ({
    ...
    submittingTicket: !1,
    featureRequest: { title: "", category: "", description: "" },
    sections: [
      {
        id: "getting-started",
    ...
    faqData: [
      ...
      },
      {
        ...
        answer:
          "Click on the theme toggle button in the top navigation bar to switch between light and dark modes. Your preference will be saved automatically.",
        open: !1,
      },
      {
        question: "What browsers are supported?",
        answer:
          "The dashboard supports all modern browsers including Chrome 90+, Firefox 88+, Safari 14+, and Edge 90+. Internet Explorer is not supported.",
        open: !1,
      },
      {
        question: "How do I set up two-factor authentication?",
        answer:
          "Go to Security settings, find the Two-Factor Authentication section, and follow the setup wizard to configure 2FA using an authenticator app or SMS.",
        open: !1,
      },
      {
        question: "Can I customize the dashboard layout?",
        answer:
          "Yes, you can customize widget arrangements, hide/show components, and adjust various layout preferences in the Settings page under the Appearance section.",
        open: !1,
      },
      {
        question: "How do I contact technical support?",
        answer:
          "You can reach our support team through live chat, email, or by submitting a support ticket. Live chat is available during business hours for immediate assistance.",
        open: !1,
      },
      {
        question: "Is there a mobile app available?",
        answer:
          "Currently, the dashboard is optimized for mobile browsers. A native mobile app is planned for future release. You can add the web app to your home screen for app-like experience.",
        open: !1,
      },
      {
        question: "How do I backup my data?",
        answer:
          "Data backups are performed automatically. You can also export your data manually from various sections of the dashboard. Contact support for full account backup options.",
        open: !1,
      },
    ],
    docCategories: [
    ...
    documentationData: {
      "getting-started": [
        {
          ...
          description:
            "Get up and running with the admin dashboard in 5 minutes",
          type: "Guide",
        ...
        },
        {
          ...
          description:
            "Understanding the main dashboard interface and navigation",
          type: "Tutorial",
      ...
      "user-management": [
        ...
        },
        {
          ...
          description:
            "Configure roles and manage user permissions effectively",
          type: "Guide",
      ...
      analytics: [
        {
          ...
          description:
            "Build and customize reports to meet your specific needs",
          type: "Tutorial",
    ...
    toggleFAQ(e) {
      this.filteredFAQ[e].open = !this.filteredFAQ[e].open;
    },
    filterFAQ() {
      ...
      const e = this.faqSearch.toLowerCase();
      (this.filteredFAQ = this.faqData.filter(
        (t) =>
          t.question.toLowerCase().includes(e) ||
          t.answer.toLowerCase().includes(e)
      )),
        this.filteredFAQ.forEach((t) => (t.open = !1));
    },
    highlightText(e, t) {
      if (!t) return e;
      const i = new RegExp(`(${t})`, "gi");
      return e.replace(i, '<span class="search-highlight">$1</span>');
    },
    selectDocCategory(e) {
      this.selectedDocCategory = e;
    },
    get selectedDocs() {
      return this.documentationData[this.selectedDocCategory] || [];
    },
    openDoc(e) {
      this.showNotification(`Opening documentation: ${e}`, "info");
    },
    openArticle(e) {
      this.showNotification(`Opening article: ${e}`, "info");
    },
    playVideo(e) {
      this.showNotification(`Playing video: ${e}`, "info");
    },
    startLiveChat() {
    ...
    showPhoneInfo() {
      alert(
        "Support Phone: +1 (555) 123-HELP\\n\\nBusiness Hours:\\nMonday - Friday: 9 AM - 6 PM EST\\nWeekend: Emergency support only"
      );
    },
    submitTicket() {
      this.validateTicketForm() &&
        ((this.submittingTicket = !0),
        setTimeout(() => {
          (this.submittingTicket = !1),
    ...
    validateTicketForm() {
      return [
        ...
        "subject",
        "description",
      ].filter((i) => !this.supportTicket[i]).length > 0
    ...
    submitFeatureRequest() {
      if (
        !this.featureRequest.title ||
        !this.featureRequest.category ||
      ...
      this.popularRequests.unshift(e),
        this.showNotification(
          "Feature request submitted successfully!",
          "success"
        ),
        (this.featureRequest = { title: "", category: "", description: "" });
    },
    voteFeature(e) {
      const t = this.popularRequests.find((i) => i.id === e);
      t &&
        !t.voted &&
        (t.votes++,
        (t.voted = !0),
    ...
    loadUserPreferences() {
      const e = localStorage.getItem("helpPreferences");
      if (e)
        try {
          const t = JSON.parse(e);
          t.lastSection && (this.activeSection = t.lastSection);
        } catch (t) {
          console.warn("Failed to load help preferences:", t);
        }
    },
    saveUserPreferences() {
      const e = {
      ...
      localStorage.setItem("helpPreferences", JSON.stringify(e));
    },
    showNotification(e, t = "info") {
      console.log(`[${t.toUpperCase()}] ${e}`),
        document.dispatchEvent(
          new CustomEvent("showNotification", {
            detail: { message: e, type: t },
          })
        );
    },
  })),
    a.data("searchComponent", () => ({
      query: "",
      results: [],
      search() {
        if (
          ...
          const e = a.$data(document.querySelector('[x-data="helpComponent"]'));
          e &&
            e.activeSection === "faq" &&
            ((e.faqSearch = this.query), e.filterFAQ());
        } else this.results = [];
      },
    })),
    a.data("themeSwitch", () => ({
