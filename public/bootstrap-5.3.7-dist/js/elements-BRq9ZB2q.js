...
document.addEventListener("alpine:init", () => {
  a.data("elementsComponent", () => ({
    ...
    filterComponents() {
      ...
      this.categoryFilter &&
        (t = t.filter((s) => s.category === this.categoryFilter)),
        (this.filteredComponents = t);
    },
    toggleView() {
      (this.viewMode = this.viewMode === "grid" ? "list" : "grid"),
        localStorage.setItem("elements-view-mode", this.viewMode);
    },
    navigateToComponent(t) {
    ...
    getComponentCount(t) {
      return this.components.filter((s) => s.category === t).length;
    },
  })),
    a.data("searchComponent", () => ({
      ...
      async search() {
        ...
        (this.isLoading = !0), await new Promise((e) => setTimeout(e, 200));
        const t = i
            .filter(
              (e) =>
            ...
            .map((e) => ({ title: e.title, url: e.url, type: "element" })),
          s = [
        ...
        (this.results = [...t, ...s].slice(0, 8)), (this.isLoading = !1);
      },
    }));
});
document.addEventListener("DOMContentLoaded", () => {
  [...document.querySelectorAll('[data-bs-toggle="tooltip"]')].map(
    (s) => new bootstrap.Tooltip(s)
  ),
    localStorage.getItem("elements-view-mode");
});
export { i as elementsData };
