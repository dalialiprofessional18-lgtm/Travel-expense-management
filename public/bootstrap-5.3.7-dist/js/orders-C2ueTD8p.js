import { m as r } from "./bootstrap.bundle.min.js";
document.addEventListener("alpine:init", () => {
  r.data("orderTable", () => ({
    ...
    isLoading: !1,
    chartsInitialized: !1,
    stats: { total: 0, pending: 0, shipped: 0, revenue: 0 },
    statusStats: [],
    init() {
      this.loadSampleData(),
        this.filterOrders(),
      ...
      this.orders = [
        {
          ...
          items: [{ name: "iPhone 14 Pro", quantity: 1, price: 999.99 }],
          itemCount: 1,
          total: 999.99,
          status: "pending",
        ...
        },
        {
          ...
          items: [{ name: "Samsung Galaxy S24", quantity: 1, price: 899.99 }],
          itemCount: 1,
          total: 899.99,
          status: "pending",
        ...
        },
        {
          ...
          items: [{ name: "Coffee Maker Deluxe", quantity: 1, price: 249.99 }],
          itemCount: 1,
          total: 249.99,
          status: "pending",
        ...
        },
        {
          ...
          items: [{ name: 'Tablet Pro 12.9"', quantity: 1, price: 1099.99 }],
          itemCount: 1,
          total: 1099.99,
          status: "delivered",
        ...
        },
        {
          ...
          items: [{ name: "AI & Machine Learning", quantity: 1, price: 79.99 }],
          itemCount: 1,
          total: 79.99,
          status: "pending",
        ...
        },
        {
          ...
          items: [{ name: "Cotton T-Shirt", quantity: 5, price: 24.99 }],
          itemCount: 5,
          total: 124.95,
          status: "shipped",
        ...
        },
        {
          ...
          items: [{ name: "MacBook Air M2", quantity: 1, price: 1199.99 }],
          itemCount: 1,
          total: 1199.99,
          status: "delivered",
    ...
    calculateStats() {
      (this.stats.total = this.orders.length),
        ...
        (this.stats.shipped = this.orders.filter(
          (t) => t.status === "shipped"
        ).length),
        (this.stats.revenue = this.orders
          .filter((t) => t.status !== "cancelled")
          .reduce((t, a) => t + a.total, 0));
      const e = {};
      this.orders.forEach((t) => {
        e[t.status] = (e[t.status] || 0) + 1;
      }),
        (this.statusStats = Object.entries(e).map(([t, a]) => ({
          name: t.charAt(0).toUpperCase() + t.slice(1),
          count: a,
          percentage: Math.round((a / this.orders.length) * 100),
          color: this.getStatusColor(t),
        })));
    },
    getStatusColor(e) {
      return (
    ...
    filterOrders() {
      (this.filteredOrders = this.orders.filter((e) => {
        const t =
          ...
          s = !this.dateFilter || this.matchesDateFilter(e.orderDate);
        return t && a && s;
      })),
        this.sortOrders(),
        (this.currentPage = 1);
    ...
    matchesDateFilter(e) {
      ...
      switch (this.dateFilter) {
        ...
        case "week":
          const s = new Date(t.getTime() - 7 * 24 * 60 * 60 * 1e3);
          return a >= s;
        case "month":
          const i = new Date(t.getTime() - 30 * 24 * 60 * 60 * 1e3);
          return a >= i;
        default:
          return !0;
      }
    ...
    sortOrders() {
      this.filteredOrders.sort((e, t) => {
        let a = e[this.sortField],
          s = t[this.sortField];
        return (
          this.sortField === "total"
            ? ((a = parseFloat(a)), (s = parseFloat(s)))
            : this.sortField === "orderDate"
            ? ((a = new Date(a)), (s = new Date(s)))
    ...
    toggleAll(e) {
      e
        ? (this.selectedOrders = this.paginatedOrders.map((t) => t.id))
        : (this.selectedOrders = []);
    },
    bulkAction(e) {
      if (this.selectedOrders.length === 0) return;
      const t = this.orders.filter((a) => this.selectedOrders.includes(a.id));
      switch (e) {
        case "processing":
          t.forEach((a) => {
        ...
        case "shipped":
          t.forEach((a) => {
            a.status === "processing" && (a.status = "shipped");
          }),
            this.showNotification("Orders marked as shipped!", "success");
          break;
        case "delivered":
          t.forEach((a) => {
            a.status === "shipped" && (a.status = "delivered");
          }),
            this.showNotification("Orders marked as delivered!", "success");
          break;
      }
      (this.selectedOrders = []), this.calculateStats();
    },
    viewOrder(e) {
    ...
    exportOrders() {
      const e =
          `data:text/csv;charset=utf-8,Order Number,Customer,Email,Items,Total,Status,Date
` +
          this.filteredOrders.map(
            (s) =>
              `"${s.orderNumber}","${s.customer.name}","${s.customer.email}","${s.itemCount}","${s.total}","${s.status}","${s.orderDate}"`
          ).join(`
`),
        t = encodeURI(e),
    ...
    initOrderTrendsChart() {
      const e = document.getElementById("orderTrendsChart");
      if (!e) {
      ...
      try {
        const t = {
          series: [
            { name: "Orders", data: [12, 19, 15, 27, 24, 32, 28] },
            {
              name: "Revenue",
        ...
        console.error("Error rendering order trends chart:", t);
      }
    },
    initStatusChart() {
      const e = document.getElementById("statusChart");
      if (!e) {
      ...
      try {
        const t = {
          series: this.statusStats.map((s) => s.count),
          chart: { type: "donut", height: 200 },
          labels: this.statusStats.map((s) => s.name),
          colors: this.statusStats.map((s) => s.color),
        ...
        console.error("Error rendering status chart:", t);
      }
    },
    get paginatedOrders() {
      const e = (this.currentPage - 1) * this.itemsPerPage,
        t = e + this.itemsPerPage;
      return this.filteredOrders.slice(e, t);
    },
    get totalPages() {
    ...
    get visiblePages() {
      ...
      const e = [];
      if ((e.push(1), this.totalPages <= 7))
        for (let t = 2; t <= this.totalPages; t++) e.push(t);
      else if (this.currentPage <= 4) {
        for (let t = 2; t <= 5; t++) e.push(t);
        e.push("..."), e.push(this.totalPages);
      } else if (this.currentPage >= this.totalPages - 3) {
        e.push("...");
        for (let t = this.totalPages - 4; t <= this.totalPages; t++) e.push(t);
      } else {
        e.push("...");
        for (let t = this.currentPage - 1; t <= this.currentPage + 1; t++)
          e.push(t);
        e.push("..."), e.push(this.totalPages);
      }
      return e;
    ...
    r.data("searchComponent", () => ({
      query: "",
      results: [],
      search() {
        console.log("Searching for:", this.query), (this.results = []);
      },
    })),
    r.data("themeSwitch", () => ({
      currentTheme: "light",
