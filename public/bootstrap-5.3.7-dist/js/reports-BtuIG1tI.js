...
document.addEventListener("alpine:init", () => {
  a.data("reportsComponent", () => ({
    ...
    updateDateRange() {
      console.log("Date range updated to:", this.dateRange), this.refreshData();
    },
    updateReportType() {
    ...
    refreshData() {
      (this.kpis.revenue = Math.floor(Math.random() * 5e4) + 1e5),
        (this.kpis.orders = Math.floor(Math.random() * 500) + 1e3),
        (this.kpis.customers = Math.floor(Math.random() * 200) + 800),
        this.chartsInitialized && this.updateCharts();
    },
    scheduleReport() {
    ...
    exportData() {
      const e = `report_${this.reportType}_${Date.now()}.${this.exportFormat}`;
      this.showNotification(`Exporting data as ${e}...`, "success");
    },
    refreshReports() {
      this.showNotification("Reports refreshed!", "success");
    },
    downloadReport(e) {
      console.log("Downloading report:", e),
        this.showNotification(`Downloading ${e.name}...`, "success");
    },
    shareReport(e) {
      console.log("Sharing report:", e),
        this.showNotification("Share functionality would open here", "info");
    },
    duplicateReport(e) {
      const t = {
        ...e,
        id: `RPT-${String(Math.floor(Math.random() * 1e3)).padStart(3, "0")}`,
        name: `${e.name} (Copy)`,
      ...
      try {
        const t = {
          series: [
            {
              name: "Revenue",
              data: [28e3, 32e3, 35e3, 41e3, 38e3, 45e3, 52e3],
            },
            {
              name: "Profit",
        ...
        console.error("Error rendering revenue trends chart:", t);
      }
    },
    initTopProductsChart() {
      ...
      try {
        ...
        console.error("Error rendering top products chart:", t);
      }
    },
    initCustomerAcquisitionChart() {
      ...
      try {
        ...
        console.error("Error rendering customer acquisition chart:", t);
      }
    },
    initRegionSalesChart() {
      ...
      try {
        ...
        console.error("Error rendering region sales chart:", t);
      }
    },
    updateCharts() {
