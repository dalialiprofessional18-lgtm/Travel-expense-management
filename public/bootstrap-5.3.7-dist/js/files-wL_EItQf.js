...
document.addEventListener("alpine:init", () => {
  o.data("filesComponent", () => ({
    ...
    loadSampleData() {
      (this.folders = [
        { id: 1, name: "Documents", fileCount: 23, icon: "bi-folder-fill" },
        { id: 2, name: "Images", fileCount: 156, icon: "bi-folder-fill" },
        { id: 3, name: "Projects", fileCount: 12, icon: "bi-folder-fill" },
        { id: 4, name: "Shared", fileCount: 8, icon: "bi-folder-fill" },
        { id: 5, name: "Archive", fileCount: 45, icon: "bi-folder-fill" },
      ]),
        (this.allFiles = [
          {
            id: 1,
        ...
        (this.recentFiles = this.allFiles
          .sort(
            (e, t) =>
              this.getModifiedTimestamp(e.modifiedDate) -
              this.getModifiedTimestamp(t.modifiedDate)
          )
          .reverse()
          .slice(0, 5)),
        (this.quickAccess = [
          ...
          },
          {
            name: "Images",
            icon: "bi-image",
            count: this.allFiles.filter((e) => e.type === "image").length,
            type: "images",
          },
          {
            name: "Documents",
            icon: "bi-file-earmark-text",
            count: this.allFiles.filter((e) => e.type === "document").length,
            type: "documents",
          },
          {
            name: "Shared",
            icon: "bi-people",
            count: this.allFiles.filter((e) => e.folder === "Shared").length,
            type: "shared",
          },
          { name: "Trash", icon: "bi-trash", count: 0, type: "trash" },
        ]);
    },
    getModifiedTimestamp(e) {
      const t = new Date();
      if (e.includes("hour")) {
        const i = parseInt(e);
        return t.getTime() - i * 60 * 60 * 1e3;
      } else if (e.includes("day")) {
        const i = parseInt(e);
        return t.getTime() - i * 24 * 60 * 60 * 1e3;
      } else if (e.includes("week")) {
        const i = parseInt(e);
        return t.getTime() - i * 7 * 24 * 60 * 60 * 1e3;
      } else if (e.includes("month")) {
        const i = parseInt(e);
        return t.getTime() - i * 30 * 24 * 60 * 60 * 1e3;
      }
      return t.getTime();
    },
    sortFiles() {
      this.currentFiles.sort((e, t) => {
        switch (this.sortBy) {
          case "name":
          ...
          case "size":
            return this.parseSize(e.size) - this.parseSize(t.size);
          case "type":
            return e.typeLabel.localeCompare(t.typeLabel);
          default:
            return 0;
        }
      });
    ...
    selectFile(e) {
      const t = this.selectedFiles.indexOf(e.id);
      t > -1 ? this.selectedFiles.splice(t, 1) : this.selectedFiles.push(e.id);
    },
    toggleFileSelection(e) {
      const t = this.selectedFiles.indexOf(e);
      t > -1 ? this.selectedFiles.splice(t, 1) : this.selectedFiles.push(e);
    },
    selectAll() {
      this.selectedFiles.length === this.currentFiles.length
        ? (this.selectedFiles = [])
        : (this.selectedFiles = this.currentFiles.map((e) => e.id));
    },
    toggleSelectAll() {
      this.selectAll();
    },
    openFolder(e) {
      (this.currentFolder = e),
        (this.currentFiles = this.allFiles.filter((t) => t.folder === e.name)),
        (this.breadcrumbs = [
          { name: "My Files", path: "/" },
    ...
    navigateToBreadcrumb(e) {
      if (((this.breadcrumbs = this.breadcrumbs.slice(0, e + 1)), e === 0))
        (this.currentFolder = null),
          (this.currentFiles = [...this.allFiles]),
          (this.breadcrumbs = [{ name: "My Files", path: "/" }]);
      else {
        const t = this.breadcrumbs[e].name,
          i = this.quickAccess.find((l) => l.name === t);
        if (i) {
          this.navigateToQuickAccessItem(i);
          return;
        }
        const s = this.folders.find((l) => l.name === t);
        s
          ? ((this.currentFolder = s),
    ...
    navigateToQuickAccess(e) {
      this.navigateToQuickAccessItem(e);
    },
    navigateToQuickAccessItem(e) {
      switch (
        ((this.breadcrumbs = [
          { name: "My Files", path: "/" },
          { name: e.name, path: `/${e.type}` },
        ]),
        (this.currentFolder = null),
        e.type)
      ) {
        case "recent":
          ...
          break;
        case "images":
          this.currentFiles = this.allFiles.filter((t) => t.type === "image");
          break;
        case "documents":
          this.currentFiles = this.allFiles.filter(
            (t) => t.type === "document"
    ...
    openFile(e) {
      typeof Swal < "u"
        ? Swal.fire({
            title: `Opening ${e.name}`,
            html: `
            <div class="text-start">
              <p><strong>📁 File:</strong> ${e.name}</p>
              <p><strong>📏 Size:</strong> ${e.size}</p>
              <p><strong>📅 Modified:</strong> ${e.modifiedDate}</p>
              <p><strong>📂 Folder:</strong> ${e.folder}</p>
              <p><strong>🏷️ Type:</strong> ${e.typeLabel}</p>
            </div>
          `,
            icon: "info",
            showCancelButton: !0,
            ...
            t.isConfirmed &&
              this.showNotification(
                `Opening ${e.name} in default application...`,
                "success"
              );
          })
        : this.showNotification(`Opening ${e.name}`, "info");
    },
    downloadFile(e) {
      this.showNotification(`📥 Downloading ${e.name}...`, "success"),
        setTimeout(() => {
          this.showNotification(
            `✅ ${e.name} downloaded successfully!`,
            "success"
          );
        }, 2e3);
    },
    downloadSelected() {
      ...
      typeof Swal < "u"
        ...
        : this.showNotification(
            `Downloading ${this.selectedFiles.length} files...`,
            "success"
          );
    },
    shareFile(e) {
    ...
    renameFile(e) {
      if (typeof Swal < "u")
        Swal.fire({
          ...
          confirmButtonColor: "var(--bs-primary)",
          inputValidator: (t) => {
            if (!t || t.trim() === "") return "Please enter a valid file name";
            if (t === e.name) return "Please enter a different name";
          },
        }).then((t) => {
        ...
        const t = prompt("Enter new file name:", e.name);
        t &&
          t !== e.name &&
          ((e.name = t),
    ...
    deleteFile(e) {
      typeof Swal < "u"
        ? Swal.fire({
            title: "Delete File",
            text: `Are you sure you want to delete "${e.name}"? This action cannot be undone.`,
            icon: "warning",
            showCancelButton: !0,
            ...
            t.isConfirmed &&
              (this.performFileDelete(e),
              this.showNotification(
                `🗑️ "${e.name}" moved to trash`,
                "success"
              ));
          })
    ...
    performFileDelete(e) {
      const t = this.currentFiles.findIndex((s) => s.id === e.id);
      t > -1 && this.currentFiles.splice(t, 1);
      const i = this.allFiles.findIndex((s) => s.id === e.id);
      i > -1 && this.allFiles.splice(i, 1),
        (this.selectedFiles = this.selectedFiles.filter((s) => s !== e.id));
    },
    deleteSelected() {
      if (this.selectedFiles.length === 0) {
        this.showNotification("❌ No files selected", "warning");
      ...
      typeof Swal < "u"
        ? Swal.fire({
            ...
            if (e.isConfirmed) {
              const t = this.selectedFiles.length;
              (this.currentFiles = this.currentFiles.filter(
                (i) => !this.selectedFiles.includes(i.id)
              )),
        ...
        : confirm(
          ...
          ((this.currentFiles = this.currentFiles.filter(
            (e) => !this.selectedFiles.includes(e.id)
          )),
          (this.allFiles = this.allFiles.filter(
            (e) => !this.selectedFiles.includes(e.id)
          )),
      ...
      typeof Swal < "u"
        ? Swal.fire({
          ...
          `,
            ...
            confirmButtonColor: "var(--bs-primary)",
            preConfirm: () => {
              const e = document.getElementById("fileUpload");
            ...
            e.isConfirmed &&
              (this.showNotification(
                `☁️ Uploading ${e.value.length} files...`,
                "success"
              ),
              setTimeout(() => {
                this.showNotification(
                  `✅ ${e.value.length} files uploaded successfully!`,
                  "success"
                );
              }, 2500));
          })
    ...
    createFolder() {
      if (typeof Swal < "u")
        Swal.fire({
          ...
          confirmButtonColor: "var(--bs-primary)",
          inputValidator: (e) => {
            if (!e || e.trim() === "") return "Please enter a folder name";
            if (
              this.folders.some((t) => t.name.toLowerCase() === e.toLowerCase())
            )
          ...
          if (e.isConfirmed && e.value) {
            ...
            this.folders.push(t),
              this.showNotification(
                `📁 Folder "${t.name}" created successfully`,
                "success"
              );
          }
        });
      else {
        const e = prompt("Enter folder name:");
        if (e) {
          const t = {
            id: this.folders.length + 1,
            name: e,
