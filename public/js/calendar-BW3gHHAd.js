import { m as h, M as v } from "../bootstrap-5.3.7-dist/bootstrap.bundle.min.js";
document.addEventListener("alpine:init", () => {
  h.data("calendarComponent", () => ({
    sidebarVisible: !1,
    currentView: "month",
    selectedDate: null,
    selectedDay: null,
    ...
    loadSampleEvents() {
      const e = new Date(),
        ...
        d = new Date(t, n + 1, 0).getDate();
      this.events = [];
      let c = 0;
      for (let o = i; o <= d && c < s.length; o++)
        if ((o - i) % 2 === 0 || o === i || o === d) {
          const l = s[c],
            D = new Date(t, n, o),
            p = `${t}-${String(n + 1).padStart(2, "0")}-${String(o).padStart(
              2,
              "0"
            )}`;
          let u, g;
          o === i
            ? ((u = this.formatTime(l.time)), (g = "Today"))
            : o === i + 1
            ? ((u = this.formatTime(l.time)), (g = "Tomorrow"))
            : ((u = this.formatTime(l.time)), (g = `${a} ${o}`)),
            this.events.push({
              id: c + 1,
              title: l.title,
              type: l.type,
      ...
      const m = n === 11 ? 0 : n + 1,
        y = n === 11 ? t + 1 : t,
        w = r[m];
      for (let o = 1; o <= 10 && c < s.length; o += 2) {
        const l = s[c],
          D = new Date(y, m, o),
          p = `${y}-${String(m + 1).padStart(2, "0")}-${String(o).padStart(
            2,
            "0"
          )}`;
        this.events.push({
          ...
          type: l.type,
          time: l.time,
          timeStr: this.formatTime(l.time),
          dateStr: `${w} ${o}`,
          description: l.description,
          date: p,
          dateObj: D,
    ...
    get currentPeriodTitle() {
      ...
      if (this.currentView === "week") {
        ...
        e.setDate(this.currentDate.getDate() - this.currentDate.getDay());
        const t = new Date(e);
        return (
    ...
    get selectedDayTitle() {
      return this.selectedDay
        ? new Date(this.selectedDay).toLocaleDateString("en-US", {
            weekday: "long",
          })
        : "";
    },
    ...
    get miniCalendarDays() {
      const e = this.miniCalendarDate.getFullYear(),
        t = this.miniCalendarDate.getMonth(),
        n = new Date(e, t, 1),
        i = new Date(n);
      i.setDate(i.getDate() - n.getDay());
      const r = [];
      for (let a = 0; a < 42; a++) {
        const s = new Date(i);
        s.setDate(i.getDate() + a);
        const d = s.toISOString().split("T")[0],
          c = this.events.some((m) => m.date === d);
        r.push({
          date: d,
          day: s.getDate(),
          isToday: this.isToday(s),
          isOtherMonth: s.getMonth() !== t,
    ...
    get calendarDays() {
      ...
      for (let a = 0; a < 42; a++) {
        ...
        s.setDate(i.getDate() + a);
        const d = s.toISOString().split("T")[0],
          c = this.getEventsForDate(d).filter((m) =>
            this.visibleTypes.includes(m.type)
          );
        r.push({
          date: d,
          day: s.getDate(),
      ...
      for (let n = 0; n < 7; n++) {
        ...
        i.setDate(e.getDate() + n),
          t.push({
            ...
            dayName: i.toLocaleDateString("en-US", { weekday: "short" }),
            dayNumber: i.getDate(),
    ...
    get upcomingEvents() {
      ...
      return this.events
        .filter((t) =>
          this.visibleTypes.includes(t.type) ? t.dateObj >= e : !1
        )
        .sort((t, n) => t.dateObj - n.dateObj)
        .slice(0, 8);
    },
    previousMonth() {
      const e = new Date(this.miniCalendarDate);
      e.setMonth(e.getMonth() - 1), (this.miniCalendarDate = e);
    },
    nextMonth() {
      const e = new Date(this.miniCalendarDate);
      e.setMonth(e.getMonth() + 1), (this.miniCalendarDate = e);
    ...
    selectDate(e) {
      (this.selectedDay = e),
        ...
        this.currentView === "day" && (this.currentDate = new Date(e));
    },
    selectDay(e) {
    ...
    isCurrentHour(e) {
      const n = new Date().getHours(),
        ...
        r = e.includes("AM");
      let a = i;
      return !r && i !== 12 ? (a += 12) : r && i === 12 && (a = 0), a === n;
    },
    getEventsForDate(e) {
      return this.events.filter((t) => t.date === e);
    },
    getEventsForDateTime(e, t) {
      return this.events.filter((n) =>
        n.date !== e ? !1 : this.eventMatchesHour(n, t)
      );
    },
    eventMatchesHour(e, t) {
      const n = parseInt(e.time.split(":")[0]),
        i = parseInt(t.split(":")[0]),
        r = t.includes("AM");
      let a = i;
      return !r && i !== 12 ? (a += 12) : r && i === 12 && (a = 0), n === a;
    },
    getDayEventCount(e) {
      return this.getEventsForDate(e).length;
    },
    getCategoryCount(e) {
      return this.events.filter((t) => t.type === e).length;
    },
    getCategoryColor(e) {
      return (
        {
    ...
    viewEvent(e) {
      this.showNotification(`Viewing event: ${e.title}`, "info");
    },
    addEvent() {
      new v(document.getElementById("addEventModal")).show();
    },
    addEventForDay(e) {
      this.selectDay(e);
      const t = h.$data(document.querySelector('[x-data*="addEventModal"]'));
      t && (t.eventData.date = e.date), this.addEvent();
    },
    addEventAtTime(e, t) {
      this.selectDate(e);
      const n = h.$data(document.querySelector('[x-data*="addEventModal"]'));
      if (n) {
        n.eventData.date = e;
        const i = parseInt(t.split(":")[0]),
          r = t.includes("AM");
        let a = i;
        !r && i !== 12 ? (a += 12) : r && i === 12 && (a = 0),
          (n.eventData.time = `${a.toString().padStart(2, "0")}:00`);
      }
      this.addEvent();
    },
    showMoreEvents(e) {
      this.selectDay(e),
        this.showNotification(`Showing all events for ${e.date}`, "info");
    },
    exportCalendar() {
    ...
    h.data("addEventModal", () => ({
      ...
      submitEvent() {
        ...
        const e = {
          ...this.eventData,
          formattedDate: new Date(this.eventData.date).toLocaleDateString(
            "en-US",
            { weekday: "long", year: "numeric", month: "long", day: "numeric" }
          ),
          formattedTime: new Date(
            `2000-01-01T${this.eventData.time}`
        ...
        console.log("Creating event:", e),
          typeof Swal < "u"
            ...
            <div class="text-start">
              ...
              <p><strong>⏱️ Duration:</strong> ${this.getDurationLabel(
                e.duration
              )}</p>
            </div>
          `,
      ...
      closeModal() {
        const e = document.getElementById("addEventModal");
        if (e) {
          const t = v.getInstance(e);
          t && t.hide();
        }
        this.resetForm();
      },
